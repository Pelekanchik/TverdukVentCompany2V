...
class AcousticsTab:
    ...
    def _build_ui(self):
        ...
        ttk.Spinbox(fan_frame, from_=100, to=50000, textvariable=self.fan_flow_var, width=12).grid(
        ...
        ttk.Spinbox(
            fan_frame, from_=10, to=5000, textvariable=self.fan_pressure_var, width=12
        ...
        ttk.Spinbox(
            fan_frame, from_=0.04, to=30, increment=0.1, textvariable=self.fan_power_var, width=12
        ...
        ttk.Spinbox(duct_frame, from_=1, to=200, textvariable=self.duct_length_var, width=12).grid(
            row=0, column=1, padx=2
        )

        ttk.Label(duct_frame, text="Ширина (мм):").grid(row=1, column=0, sticky=tk.W, pady=2)
        self.duct_width_var = tk.DoubleVar(value=300)
        ttk.Spinbox(duct_frame, from_=50, to=2000, textvariable=self.duct_width_var, width=12).grid(
            row=1, column=1, padx=2
        )
        ...
        ttk.Spinbox(
            duct_frame, from_=50, to=2000, textvariable=self.duct_height_var, width=12
        ).grid(row=2, column=1, padx=2)

        ttk.Label(duct_frame, text="Швидкість (м/с):").grid(row=3, column=0, sticky=tk.W, pady=2)
        self.velocity_var = tk.DoubleVar(value=9.3)
        ttk.Spinbox(duct_frame, from_=1, to=25, textvariable=self.velocity_var, width=12).grid(
            row=3, column=1, padx=2
        )

        ttk.Label(duct_frame, text="Відводів:").grid(row=4, column=0, sticky=tk.W, pady=2)
        self.elbows_var = tk.IntVar(value=2)
        ttk.Spinbox(duct_frame, from_=0, to=20, textvariable=self.elbows_var, width=12).grid(
        ...
        ttk.Spinbox(duct_frame, from_=0, to=10, textvariable=self.tees_var, width=12).grid(
            row=5, column=1, padx=2
        )
        ...
        ttk.Spinbox(
            room_frame, from_=10, to=10000, textvariable=self.room_volume_var, width=12
        ...
        ttk.Spinbox(room_frame, from_=1, to=500, textvariable=self.absorption_var, width=12).grid(
        ...
        ttk.Button(left, text="📊 Розрахувати шум", command=self._calculate).grid(
            row=4, column=0, pady=5, sticky=tk.EW
        )
        ttk.Button(left, text="🔇 Підібрати шумоглушник", command=self._select_silencer).grid(
            row=5, column=0, pady=3, sticky=tk.EW
        )
        ttk.Button(left, text="🔄 Очистити", command=self._clear).grid(
            row=6, column=0, pady=3, sticky=tk.EW
        )
        ...
        for i, (key, text, unit) in enumerate(result_fields):
            ttk.Label(result_frame, text=text, font=("Arial", 10)).grid(
            ...
            ttk.Label(result_frame, text=unit, font=("Arial", 9), foreground="#666").grid(
    ...
    def _calculate(self):
        ...
        try:
            ...
            if excess > 0:
                ...
                self.status_var.set(
                    f"❌ ПЕРЕВИЩЕННЯ НОРМИ на {excess:.1f} дБА! Потрібен шумоглушник."
                )
                self.status_var.set(
                    "❌ ПЕРЕВИЩЕННЯ НОРМИ на " + f"{excess:.1f}" + " дБА! Потрібен шумоглушник."
                )
            else:
    ...
    def _select_silencer(self):
        ...
        if silencer:
            ...
            if new_excess > 0:
                ...
                msg = (
                    "⚠️ Рекомендовано: " + silencer.name + " (зниження " + f"{att:.0f}" + " дБ). "
                ...
                msg = (
                    ...
                    + f"{att:.0f}"
                    + " дБ, "
                    + f"{silencer.pressure_drop_pa:.0f}"
                    + " Па). "
