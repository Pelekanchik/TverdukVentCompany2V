...
class Project3DPreview:
    ...
    def _build_ui(self):
        ...
        self.floor_combo = ttk.Combobox(
            ctrl, textvariable=self.floor_var, values=["Всі поверхи"], state="readonly", width=16
        )
        ...
        ttk.Checkbutton(
            ctrl, text="Архітектура", variable=self.arch_var, command=self.refresh
        ).pack(side=tk.LEFT, padx=2)
        self.vent_var = tk.BooleanVar(value=True)
        ttk.Checkbutton(ctrl, text="Вентиляція", variable=self.vent_var, command=self.refresh).pack(
            side=tk.LEFT, padx=2
        )
        self.eq_var = tk.BooleanVar(value=True)
        ttk.Checkbutton(ctrl, text="Обладнання", variable=self.eq_var, command=self.refresh).pack(
            side=tk.LEFT, padx=2
        )
        self.lbl_var = tk.BooleanVar(value=True)
        ttk.Checkbutton(ctrl, text="Підписи", variable=self.lbl_var, command=self.refresh).pack(
            side=tk.LEFT, padx=2
        )
        self.coll_var = tk.BooleanVar(value=True)
        ttk.Checkbutton(
            ctrl, text="⚠️ Зіткнення", variable=self.coll_var, command=self.refresh
        ).pack(side=tk.LEFT, padx=2)
        ttk.Separator(ctrl, orient=tk.VERTICAL).pack(side=tk.LEFT, fill=tk.Y, padx=5)
        ...
        ttk.Button(ctrl, text="↗️ Ізометрія", command=lambda: self._set_view(25, -60)).pack(
            side=tk.LEFT, padx=2
        )
        ttk.Separator(ctrl, orient=tk.VERTICAL).pack(side=tk.LEFT, fill=tk.Y, padx=5)
    ...
    def _print(self):
        try:
            with tempfile.NamedTemporaryFile(suffix=".pdf", delete=False) as tmp:
                tmp_path = tmp.name
                self.figure.savefig(
                    tmp_path, dpi=300, bbox_inches="tight", facecolor=self.figure.get_facecolor()
                )
                if os.name == "nt":
        ...
        if show_arch:
            for floor in self.project.arch_context.floors:
                ...
                for opening in floor.openings:
                    ...
                    all_y.extend(
                        [
                            opening.position.y - opening.width / 2,
                            opening.position.y + opening.width / 2,
                        ]
                    )
                    all_z.extend(
    ...
    def _draw_wall(self, wall, show_labels: bool):
        ...
        verts = [
            ...
            [
                Point3D(p1.x, p1.y, h).to_tuple(),
                Point3D(p2.x, p2.y, h).to_tuple(),
                Point3D(p3.x, p3.y, h).to_tuple(),
                Point3D(p4.x, p4.y, h).to_tuple(),
            ],
            [
                p1.to_tuple(),
                p4.to_tuple(),
        ...
        poly3d = Poly3DCollection(
            verts, alpha=0.35, facecolor=color, edgecolor=color, linewidth=0.3
        )
    ...
    def _draw_opening(self, opening):
        ...
        poly3d = Poly3DCollection(
            verts, alpha=0.6, facecolor=self.COLORS["opening"], edgecolor="#cc0000", linewidth=1
        )
        self.ax.add_collection3d(poly3d)
    ...
    def _draw_duct_segment(
        ...
        if is_collision and show_labels:
            ...
            self.ax.text(
                ...
                fontweight="bold",
                bbox=dict(
                    boxstyle="round,pad=0.3", facecolor="#ffcccc", alpha=0.9, edgecolor="#ff0000"
                ),
            )
    ...
    def _draw_equipment(self, eq, show_labels: bool, is_collision: bool = False):
        ...
        poly3d = Poly3DCollection(
            verts, alpha=alpha, facecolor=color, edgecolor=edge, linewidth=2 if is_collision else 1
        )
        self.ax.add_collection3d(poly3d)
        if is_collision and show_labels:
            self.ax.text(
