...
class ProductDialog(QDialog):
    ...
    def _update_project_label(self):
        ...
        """Оновити мітку активного проєкту."""

        parent = self.parent()

        if parent and hasattr(parent, "main_window") and parent.main_window:

            project_id = parent.main_window.active_project_id

            if project_id:

                self.lbl_project.setText(f"📁 Проєкт #{project_id}")

            else:

                self.lbl_project.setText("⚠️ Не вибрано проєкт! Виберіть у вкладці 'Проєкти'")

                self.lbl_project.setStyleSheet("color: #f44336; font-size: 13px; font-weight: bold; padding: 4px;")
